...
export function MapView({
  ...
  useEffect(() => {
    ...
    const addLayers = () => {
      ...
      /**
       * The basemap's first label layer — everything this app paints on the
       * ground is inserted underneath it.
       *
       * Place names are how a reader knows *where* a hexagon is, so a hexagon
       * drawn over "Muang Khoun" costs more than it shows: the colour is
       * readable anywhere on the shape, the name only where the basemap put
       * it. Passing this as `beforeId` puts the whole ground stack — shaded
       * areas, hexagons, the mask, the borders — below every symbol the style
       * carries, while the things that represent objects in the world (masts,
       * collectors, links) stay above, because those are the map's subject
       * rather than its background.
       *
       * `undefined` when the fallback style has no symbol layer at all, which
       * MapLibre reads as "on top" — the previous behaviour, unchanged.
       */
      const belowLabels = basemapLayers.current.find(
        (layer) => layer.type === "symbol" && instance.getLayer(layer.id),
      )?.id;
