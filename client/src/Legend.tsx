...
export function Legend({
  ...
  t: Strings;
  /**
   * The collector dot, the per-network mast colours, and the weak-area mark
   * all describe layers the public preview never draws — it is fed no
   * collectors and no cells (see site/PublicMap.tsx). Explaining a symbol
   * that cannot appear is worse than not mentioning it, so the public map
   * passes false and keeps only the five-state key, which is the one thing
   * every version of this map actually shows.
   */
      ...
      <div className="legend-body">
        ...
        <div className="legend-note">
          {showOperationalNotes && (
            <>
              {/* The dot that prompted "why has a grey core inside?" — it sits
                  at the centre of the hexagon a phone last reported from, so
                  without a line here it reads as part of the tile rather than
                  over it. */}
              <p>
                <span className="swatch swatch-collector" /> {t.legendCollector}
              </p>
              {/* Which colour belongs to which network. Without this a reader
                  sees four kinds of dot and no way to tell whose mast is
                  whose. */}
              ...
              {/* The circle that prompted "there is a tower here, so why is
                  the signal weak?" — read as a coverage footprint, it is the
                  opposite: how badly the position is pinned down. The short
                  form stays on screen because the misreading happens at a
                  glance; only the reasoning folds away. */}
              <p>
          ...
          {showOperationalNotes && (
            <details className="legend-more">
              ...
              {/* Without this the shading looks like a quality scale rather
                  than a confidence one, and a faintly green district reads as
                  slightly worse coverage instead of barely surveyed. */}
